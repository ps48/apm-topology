...
export const useCollapseDescendants = () => {
    ...
    const { getIncomingEdgesCount, getOutgoingEdges, getIncomingEdges, getChildrenNodes, hasOutgoingEdges } =
    ...
    const processChildren = useCallback(
        (nodeId: string, hideNodesIds: Set<string>, hideEdgesIds: Set<string>) => {
            ...
            const children = getChildrenNodes(nodeId, Visibility.Visible).sort((a, b) => {
                return (
                    getIncomingEdgesCount(a.id, Visibility.Visible) - getIncomingEdgesCount(b.id, Visibility.Visible)
                );
            });
                ...
                if (hideNodesIds.has(node.id)) {
                    return {
                        ...
                        data: {
                            ...
                            isCollapsed: hasOutgoingEdges(node.id) && hasOutgoingEdges(node.id, Visibility.Hidden),
                        },
                    };
                }
