...
export class TopologyNodeModel {
    ...
    get isInstrumented(): boolean {
        ...
        return !!attributes?.some(
            (attr: Record<string, string>) => attr.InstrumentationType && attr.InstrumentationType === 'INSTRUMENTED',
        );
    }
    ...
    get numberOfServices(): number {
        ...
        const attribute = this.node.AttributeMaps?.find(
            (attr: Record<string, string>) => !!attr?.InstrumentedServiceCount || !!attr?.UninstrumentedServiceCount,
        );
    ...
    get numberOfUninstrumentedServices(): number {
        ...
        const count =
            this.node.AttributeMaps?.find((attr: Record<string, string>) => !!attr?.UninstrumentedServiceCount)
    ...
    get isDirectService(): boolean {
        ...
        return !this.node.AttributeMaps?.find((attr: Record<string, string>) => attr?.isDirectService === 'false');
    }
