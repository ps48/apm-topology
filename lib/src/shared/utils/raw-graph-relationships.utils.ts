...
export function getAggregateIds(nodes: CelestialNode[]): string[] {
    const aggregatedIds = nodes.filter((n) => !!n.data.aggregatedNodeId).map((n) => n.data.aggregatedNodeId!);
 ...
 */
export function getNodeCollectionsForAggregateIds(aggregateIds: string[], nodes: CelestialNode[]): CelestialNode[][] {
