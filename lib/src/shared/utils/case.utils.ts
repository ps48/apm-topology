...
export function titleCaseToSentenceCase(input: string, exceptions: string[] = []): string {
    ...
    const processedSegments = applyExceptions(mergeAcronyms(splitAtCapitals(input)), processedExceptions);
 ...
 */
const isSingleCapitalLetter = (segment: string): boolean => segment.length === 1 && isUpperCase(segment);
 ...
 * // ['API', 'Gateway']
 */
function replacePattern(segments: string[], startIndex: number, patternLength: number, replacement: string): string[] {
    return [...segments.slice(0, startIndex), replacement, ...segments.slice(startIndex + patternLength)];
}
