...
const classifyNodes = (
    nodes: CelestialNode[],
    edges: CelestialEdge[],
): { connectedNodes: CelestialNode[]; disconnectedNodes: CelestialNode[]; parentNodes: CelestialNode[] } => {
    ...
    return {
        connectedNodes: nodes.filter((node) => !parentNodes.includes(node) && connectedNodeIds.has(node.id)),
        disconnectedNodes: nodes.filter((node) => !parentNodes.includes(node) && !connectedNodeIds.has(node.id)),
        parentNodes
    };
};
 ...
 */
const processConnectedNodes = (nodes: CelestialNode[], graph: Dagre.graphlib.Graph): CelestialNode[] => {
...
const arrangeParentNodes = (
    ...
    return parentNodes.map(parentNode => {
        ...
        const nodeChildren = childNodes.filter(
        ...
        if (nodeChildren.length > 0) {
            ...
            const nonChildNodes = childNodes.filter(
                childNode => childNode.parentId !== parentNode.id
            );
            ...
            if (nodesInsideParent.length > 0) {
                ...
                const maxXOfConflictingNodes = Math.max(...nodesInsideParent.map(node => {
                    const nodeX = node.position?.x ?? 0;
                    const nodeWidth = node.width ? Number(node.width) : config.nodeWidth;
...
export const useCelestialLayout = (defaultOptions?: LayoutOptions) => {
    ...
    const getLaidOutElements = useCallback(
        (nodes: CelestialNode[], edges: CelestialEdge[], options?: LayoutOptions) => {
            ...
            connectedNodes.filter((node) => !node.hidden).forEach((node) => {
            ...
            edges.filter((edge) => !edge.hidden).forEach((edge) => {
            ...
            const allLaidOutNodes = [...adjustedParentNodes, ...laidOutConnectedNodes, ...adjustedDisconnectedNodes];
