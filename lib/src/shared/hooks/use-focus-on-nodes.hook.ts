        ...
        (nodesInFocus: CelestialNode[], layoutNodes: CelestialNode[], delay: number = 100) => {
            ...
            setTimeout(() => {
                ...
                if (filteredLayoutNodes.length > 0) {
                    ...
                    const maxX = Math.max(...filteredLayoutNodes.map((node) => node.position.x + (node.width ?? 272))); // 272 is default node width
                    const minY = Math.min(...filteredLayoutNodes.map((node) => node.position.y));
                    const maxY = Math.max(...filteredLayoutNodes.map((node) => node.position.y + (node.height ?? 156))); // 156 is default node height
