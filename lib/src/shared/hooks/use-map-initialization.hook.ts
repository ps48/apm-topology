 ...
 */
function applyNodesFocus(nodes: CelestialNode[], edges: CelestialEdge[], nodesInFocus?: CelestialNode[]) {
 ...
 * @returns Modified edge or original edge if no changes needed
 */
function rewireIncomingEdgesToStackTop(edge: CelestialEdge, context: CollapseContext): CelestialEdge {
    if (!context.parentIds.includes(edge.source) && context.stackSiblingIds.includes(edge.target)) {
        return {
            ...edge,
 ...
 * @returns Modified edge or original edge if no changes needed
 */
function hideStackSiblingToChildEdges(edge: CelestialEdge, context: CollapseContext): CelestialEdge {
 ...
 * @returns Modified edge or original edge if no changes needed
 */
function rewireOutgoingEdgesFromStackTop(edge: CelestialEdge, context: CollapseContext): CelestialEdge {
    if (context.stackSiblingIds.includes(edge.source) && !context.childrenIds.includes(edge.target)) {
        return {
            ...edge,
...
function collapseAllExcept(
    ...
    const updatedNodes = nodes.map((node) => (doNoCollapseList.has(node.id) ? node : { ...node, hidden: true }));
