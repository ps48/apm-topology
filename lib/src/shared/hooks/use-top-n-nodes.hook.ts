 ...
 * ```
 */
export const useTopNNodes = ({
    nodes,
    edges,
    topN,
}: UseTopNNodesProps): CelestialNodes => {
    return useMemo(() => {
...
function performTopologicalTraversal(
    ...
    rootNodes.forEach((node) => {
        if (!visited.has(node.id) && (isNil(node.data?.aggregatedNodeId) || !visited.has(node.data?.aggregatedNodeId))) {
