...
vi.mock('./use-legend-position.hook', () => ({
    useLegendPosition: vi.fn().mockReturnValue({ top: 100, right: 200 }),
}));
...
describe('useLegend', () => {
    ...
    it('should toggle isOpen state when onToggle is called', () => {
        ...
        act(() => {
            result.current.onToggle({ stopPropagation: mockStopPropagation } as unknown as React.MouseEvent);
        });
    ...
    it('should calculate position using useLegendPosition', () => {
        const mockedUseLegendPosition = useLegendPosition as unknown as ReturnType<typeof vi.fn>;
        const { result } = renderHook(() => useLegend());
