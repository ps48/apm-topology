...
const generateMetrics = (requests: number, isHighError: boolean): Metrics => {
    const errorRate = isHighError
        ? getRandomNumber(METRICS_THRESHOLDS.MIN_HIGH_ERROR_RATE, METRICS_THRESHOLDS.MAX_HIGH_ERROR_RATE)
        : getRandomNumber(0, METRICS_THRESHOLDS.MAX_LOW_ERROR_RATE);

    const faults5xx = Math.floor(requests * (errorRate / 100));
    const maxErrorRate = isHighError ? METRICS_THRESHOLDS.HIGH_4XX_RATE : METRICS_THRESHOLDS.LOW_4XX_RATE;
...
export const createInitialNodes = (count: number, config: Partial<GridConfig> = {}): Node<CelestialCardProps>[] => {
